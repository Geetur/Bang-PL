# after this pass we are guaranteed to have a completely syntactically valid bang program.
    ...
    def blockenize(self):
        ...
        for node in self.expression_nodes:
            ...
            if type(node) in control_flow_nodes:
                stack.append(node)
            elif type(node) == EndNode:
                #if not stack or type(stack[-1]) == FunctionNode:
                    #raise ParserError(self.file, "missing matching construct", node.meta_data.line, node.meta_data.column_start, node.meta_data.column_end)
                construct = stack.pop()
                if type(construct) in dependant_nodes:
                    if not stack or (type(stack[-1]) != IFNode) :
                        raise ParserError(self.file, f"This construct is dependant on an if statement", construct.meta_data.line, construct.meta_data.column_start, construct.meta_data.column_end)
                    if type(construct) == ElifNode:
                ...
                if not any(type(item) == FunctionNode for item in stack):
                    raise ParserError(self.file, "missing matching construct", node.meta_data.line, node.meta_data.column_start, node.meta_data.column_end)
                if not stack:
        ...
        if stack:
            ...
            raise ParserError(self.file, "missing matching construct", missing_end_node.meta_data.line, missing_end_node.meta_data.column_start, missing_end_node.meta_data.column_end)
